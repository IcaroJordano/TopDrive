...
export default function SelectMult({ Title }: { Title: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [itenSelected, setItenSelected] = useState<string[]>([]);

  return (
    <div className="relative w-full z-50">
      <button
        className="w-full  border-none bg-neutral-100 rounded-md px-4 py-3 text-left border text-xs text-gray-600"
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center justify-between">
          {itenSelected.length > 0 ? "Selecionado" : Title}{" "}
          <IoMdArrowDropdown />
        </div>
